...
from kotonoha.playback.coordinator import PlaybackCoordinator
...
class _FakeSession:
    ...
    async def position(self, _player):
        return None
...
def _coordinator(players, *, lock=""):
    ...
    coordinator = PlaybackCoordinator(session=_FakeSession(players))
...
async def test_coordinator_prefers_complete_metadata_over_alphabetical():
    ...
    coordinator = _coordinator(players)

    await coordinator.poll_once(now=0.0)

    assert coordinator.current_player_name == "org.mpris.MediaPlayer2.plasma-browser-integration"


async def test_coordinator_prefers_player_that_started_recently():
    ...
    await coordinator.poll_once(now=0.0)
    assert coordinator.current_player_name == "org.mpris.MediaPlayer2.old"

    players["org.mpris.MediaPlayer2.new"] = new
    await coordinator.poll_once(now=20.0)

    assert coordinator.current_player_name == "org.mpris.MediaPlayer2.new"


async def test_coordinator_keeps_current_when_new_player_is_within_recency_margin():
    old = ("old", "Playing", TrackInfo("Song", "Artist", "Album", 180.0, "/old"))
    new = ("new", "Playing", TrackInfo("Song", "Artist", "Album", 180.0, "/new"))
    players = {"org.mpris.MediaPlayer2.old": old}
    coordinator = _coordinator(players)
    ...
    players["org.mpris.MediaPlayer2.new"] = new
    await coordinator.poll_once(now=RECENT_PLAYER_MARGIN / 2)

    assert coordinator.current_player_name == "org.mpris.MediaPlayer2.old"


async def test_coordinator_lock_beats_recently_started_rival():
    ...
    coordinator = _coordinator(players, lock="org.mpris.MediaPlayer2.locked")

    await coordinator.poll_once(now=0.0)
    assert coordinator.current_player_name == "org.mpris.MediaPlayer2.locked"
...
async def test_coordinator_clears_selection_when_player_vanishes():
    vanished = ("vanished", "Playing", TrackInfo("Song", "Artist", "Album", 180.0, "/vanished"))
    players = {"org.mpris.MediaPlayer2.vanished": vanished}
    coordinator = _coordinator(players)

    await coordinator.poll_once(now=0.0)
    assert coordinator.current_player_name == "org.mpris.MediaPlayer2.vanished"

    players.clear()
    await coordinator.poll_once(now=20.0)
    assert coordinator.current_player_name is None


async def test_coordinator_lock_beats_more_complete_rival():
    locked = ("locked", "Playing", TrackInfo("Song", "", "", 180.0, "/locked"))
    rival = ("rival", "Playing", TrackInfo("Song", "Artist", "Album", 180.0, "/rival"))
    players = {
...
async def test_coordinator_absent_lock_falls_back_to_automatic():
    rival = ("rival", "Playing", TrackInfo("Song", "Artist", "Album", 180.0, "/rival"))
    players = {"org.mpris.MediaPlayer2.rival": rival}
    coordinator = _coordinator(players, lock="org.mpris.MediaPlayer2.closed")

    await coordinator.poll_once(now=0.0)

    assert coordinator.current_player_name == "org.mpris.MediaPlayer2.rival"


async def test_coordinator_falls_back_to_only_source():
    ...
    assert coordinator.current_player_name == "org.mpris.MediaPlayer2.chromium.instance1"
...
async def test_coordinator_available_players_reports_track_status_and_automatic_choice():
    playing_metadata = {
        "xesam:title": "Song",
        "xesam:artist": ["Artist"],
    ...
    coordinator = PlaybackCoordinator(session=MprisSession(bus=_DiscoveryBus(players)))
...
async def test_coordinator_picker_marks_the_player_the_poll_would_follow():
...
async def test_coordinator_source_with_no_artist_never_wins_on_recency():
