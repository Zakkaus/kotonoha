...
- `user_impact`: The Cider plugin is no longer required for the current path. Users with API authentication disabled can connect without a token; users with authentication enabled can set the token in Settings -> Sources, where it is stored in the system keyring. Lyrics timing remains smooth without polling Cider for every display frame.
