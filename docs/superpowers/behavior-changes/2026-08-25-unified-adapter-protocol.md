...
- `new_contract`: Versioned `kotonoha.adapter` messages distinguish `snapshot` and `clock`. A snapshot carries a normalized playback observation and a complete timed lyric document. Current-line selection, interlude detection, and surrounding context are produced only by `LyricsPresentationAdapter`; Qt receives the result through `QtDisplayPublisher`.
