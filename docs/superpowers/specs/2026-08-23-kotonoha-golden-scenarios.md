...
| `resolution.default-ordered-first` | sidecar、embedded、exact hint、Cider 和 network 同时有候选；policy 为 `ordered_first` 或 `best_confidence`。 | 默认按来源顺序；`best_confidence` 显式选择后才竞争；cache 只加速所属来源，不改变优先级。 | `docs/superpowers/behavior-changes/2026-08-23-resolution-policy.md` | target |
