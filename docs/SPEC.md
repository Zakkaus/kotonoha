...
- Python 3.11+，Ruff 120 列，公共函数加类型注解，结构化数据用 `dataclass`。
...
4. **鼠标穿透 + HUD 锁**
   ...
   - 因穿透时 HUD 自身也不接收点击，**解锁入口**放在托盘（左键单击托盘图标或菜单项即可解锁）。拖动落点会写回输出、`margin_edge` 和相对目标屏幕中心的 `margin_x`；不会把虚拟桌面全局坐标写入 `margin_x`。
5. **整体不透明度**
